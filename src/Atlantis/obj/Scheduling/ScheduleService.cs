...
namespace Followme.AspNet.Core.FastCommon.Scheduling
{
    public class ScheduleService : IScheduleService
    {
        ...
        public void StartTask(Worker worker)
        {
            StartTask(worker.Name, worker.Process, worker.ScheduleInterval,worker.ScheduleInterval);
            _logger.Info($"The task is started! task name: {worker.Name}, task execute cycle: {worker.Option.Interval} {worker.Option.Cycle}, schedule check cycle: {worker.ScheduleInterval/1000} seconds");
        }
