                ...
                foreach(var method in methods)
                {
                    ...
                    if(method.ReturnType==typeof(void))
                    {
                        noResult.AppendLine($@"if(string.Equals(message.GetTypeFullName(),""{parameters[0].ParameterType.FullName}""))
                                    {{
                                        return (m)=>{{ObjectContainer.Resolve<{type.Name}>().{method.Name}(message as {parameters[0].ParameterType.FullName});}} ;   
                                    }}");
                    }
                    else
                    {
                        needResult.AppendLine($@"if(string.Equals(message.GetTypeFullName(),""{parameters[0].ParameterType.FullName}""))
                                    {{
                                        return (m)=>{{return ObjectContainer.Resolve<{type.Name}>().{method.Name}(message as {parameters[0].ParameterType.FullName}) as TMessageResult;}} ;   
                                    }}");
                    }
