...
namespace Followme.AspNet.Core.FastCommon.Configurations
{
    public static class GrpcServerConfigurationExetension
    {
        ...
        public static Configuration RegisterGrpcServer(this Configuration configuration, string namespaceName, string serviceName, string settingConfigName, bool isGeneralProtoFile = true)
        {
            ...
            RegisterBussinessCode(configuration, namespaceName, serviceName, isGeneralProtoFile);
            
            var serviceRegister=new ServiceRegister(configuration.GetAspNetCoreConfiguration());
        ...
        private static void RegisterBussinessCode(Configuration configuration, string namespaceName, string serviceName, bool isGeneralProtoFile)
        {
            ...
            if (isGeneralProtoFile)
            {
                ...
                protoStr.AppendLine($"package {namespaceName};\n");
            ...
            void CreateCallCode(MethodInfo method, ParameterInfo parameter,Type[] baseInterfaces)
            {
                ...
                codeClass.CreateMember($"{method.Name}",
                                            $@"request.SetMessageExecutingType({codeMessageType});
                                               request.SetTypeFullName(""{parameter.ParameterType.FullName}"");
                                               return _messageServicer.ProcessAsync<{parameter.ParameterType.Name},{method.ReturnType.Name}>(request,context);",
                                            $"Task<{method.ReturnType.Name}>",
                                            new CodeParameter[]
                                            {
                                                 new CodeParameter(parameter.ParameterType.Name, "request"),
                                                 new CodeParameter("ServerCallContext","context")
                                            },
                                            new CodeMemberAttribute("public", "virtual"))
                            .AddRefence($"using {parameter.ParameterType.Namespace};")
                            .AddRefence($"using {method.ReturnType.Namespace};");
            }

            void CreateGrpcCallCode(MethodInfo method, ParameterInfo parameter)
            {
                bindServicesCode.AppendLine($@".AddMethod(new Method<{parameter.ParameterType.Name},{method.ReturnType.Name}>(
                                            MethodType.Unary,
                                            ...
                                            new Marshaller<{method.ReturnType.Name}>(
                                                _binarySerializer.Serialize,
                                                _binarySerializer.Deserialize<{method.ReturnType.Name}>)
                                            ),
                                        {method.Name})");
                CodeBuilder.Instance.AddAssemblyRefence(parameter.ParameterType.Assembly.Location)
                            .AddAssemblyRefence(method.ReturnType.Assembly.Location);
            }

            void CreateProtoCode(MethodInfo method, ParameterInfo parameter)
            {
                protoServiceCode.AppendLine();
                protoServiceCode.AppendLine($"\trpc {method.Name}({parameter.ParameterType.Name}) returns({method.ReturnType.Name});");
                ...
                if (!_messages.Contains(method.ReturnType.Name))
                {
                    protoMessageCode.AppendLine(CreateProtoMessageCode(method.ReturnType));
                    _messages.Add(method.ReturnType.Name);
                }
            }
            ...
            //    Serializer.GetProto(messageType);
                ...
                int index = 1;
                foreach (var item in properties.OrderBy(p => p.Name,new ProtoMessageNameCompare()))
                {
